...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
    {
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("appid", inputObj.API_APPID);//公众账号ID
            inputObj.SetValue("mch_id", inputObj.API_MCHID);//商户号
            ...
            //将xml格式的数据转化为对象以返回
            WxPayData result = new WxPayData();
            result.API_APPID = inputObj.API_APPID;
            result.API_KEY = inputObj.API_KEY;
            result.API_MCHID = inputObj.API_MCHID;
            result.FromXml(response);
        ...
        * 
        * 发微信普通红包
        * @param WxPaydata inputObj 提交给发微信普通红包API的参数
        * @param int timeOut 超时时间
        * @throws WxPayException
        * @return 成功时返回，其他抛异常
        * 开发文档：https://dwz.cn/Fn1IqHyo
        */
        public static WxPayData SendRedPack(WxPayData inputObj, int timeOut = 6)
        {
            ...
            inputObj.SetValue("mch_id", WxPayConfig.MCHID);//商户号
            inputObj.SetValue("client_ip", WxPayConfig.IP);//终端ip	  	    
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串
            ...
            Log.Debug("WxPayApi", "SendRedPack request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);
            Log.Debug("WxPayApi", "SendRedPack response : " + response);
